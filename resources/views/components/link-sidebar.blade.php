<a class="group" href="{{ $href }}">
  <div
    class="@if (in_array(Request::segment(2), [$id ?? ''])) {{ 'bg-[#000000]' }} @endif text-white py-2 px-4 rounded-0 font-Urbanist_Regular cursor-pointer text-[16px] border-none w-64 flex justify-between items-center group-hover:bg-[#000000]">
    <p
      class="font-medium text-[16px] text-black group-hover:text-white @if (in_array(Request::segment(2), [$id ?? ''])) {{ 'text-white' }} @endif">
      {{ $slot }}
    </p>
    <span>
